(() => {
  ...
  function title() {
    ...
    show(`<div class="center"><p class="eyebrow">世界旅行クイズ</p><h1>国旗しりとり</h1><p class="lead">3つのライフで、国旗から国を選ぼう。国は再利用されるのでエンドレスで遊べます。</p><div class="modeGrid"><button data-mode="solo">1人で遊ぶ</button><button data-mode="local">みんなで対戦</button><button data-mode="cpu">コンピュータと対戦</button></div></div>`);
  }
  function setup(mode) {
  ...
  function learn() { if(state.ui!=='result') return ''; const c=state.answer, feature=(c.famousFoods||[])[0]||(c.famousPlaces||[])[0]; return `<aside class="learnSheet"><b>${c.name}</b>${c.capital?`<span>首都：${c.capital}</span>`:''}${feature?`<span>有名なもの：${feature}</span>`:''}${c.trivia?`<p>${c.trivia}</p>`:''}<button id="more">${state.more?'閉じる':'もっと見る'}</button>${state.more?`<div class="details">${c.population?`人口：約${c.population}（${c.populationYear}年）<br>`:''}${c.areaKm2?`面積：約${Number(c.areaKm2).toLocaleString() }km²<br>`:''}${c.language?`言語：${c.language}<br>`:''}${c.currency?`通貨：${c.currency}<br>`:''}${(c.specialties||[]).length?`名産・産業：${c.specialties.join('、')}`:''}</div>`:''}<button id="next">次へ</button></aside>`; }
  function render() {
  ...
  function resolve(id) { clearTimer(); const p=state.players[state.turn], ok=id===state.answer.id; state.picked=id; state.ui='result'; if(ok){p.score++;p.streak++}else{p.life--;p.streak=0} render(); scheduleAdvance(ok?2500:4000); }
  function scheduleAdvance(delay) { clearTimer(); if(!state.more) timer=window.setTimeout(advance,delay||2500); }
